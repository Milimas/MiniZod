...
describe("StringSchema", () => {
  ...
  it("should set default value", () => {
    ...
    expect(schema.toJSON().value).toBe("test");
  });
  ...
  it("should set max", () => {
    const schema = s.string().max(50);
    expect(schema.toJSON().max).toBe(50);
  });

  it("should set min", () => {
    const schema = s.string().min(5);
    expect(schema.toJSON().min).toBe(5);
  });
  ...
  it("should support method chaining", () => {
    const schema = s.string().placeholder("test").max(100).min(10);
    const json = schema.toJSON();
    expect(json.placeholder).toBe("test");
    expect(json.max).toBe(100);
    expect(json.min).toBe(10);
  });
});
...
describe("EmailSchema", () => {
  ...
  it("should set default value", () => {
    ...
    expect(schema.toJSON().value).toBe("test@example.com");
  });
